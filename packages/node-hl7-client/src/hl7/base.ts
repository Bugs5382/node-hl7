...
export class HL7_BASE extends EventEmitter implements HL7_SPEC {
  ...
  protected _validatorSetValue(
    ...
    if (rules !== undefined && !this._validatorIsVersionCompatible(rules.hl7Support)) {
      return [];
    }

    const normalized = this._validatorNormalize(value);
    this._validatorCheckDependency(errors, normalizedRules.dependsOn, fieldPath);
  ...
  private _validatorCheckValue(
    ...
    if (rules.type === "number") {
      ...
      if (isNaN(num)) {
        this._validatorThrowError(errors, `Field ${fieldPath} must be a number`);
    ...
    if (typeof rules.length === "number" && rules.type !== "date" && len !== rules.length) {
      this._validatorThrowError(
        errors,
